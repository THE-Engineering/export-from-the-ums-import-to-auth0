...
function getUserByUidSql (DRUPAL_UID) {
  return `
    WITH
      Users AS (
        SELECT
    ...
    WHERE users.uid = ${DRUPAL_UID}
    ORDER BY users.uid
`
}

export default async function getUserByUid (DRUPAL_UID) {
  const querySql = getUserByUidSql(DRUPAL_UID).trim() + ';'
