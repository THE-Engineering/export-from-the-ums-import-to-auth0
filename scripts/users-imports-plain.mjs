...
async function app () {
  ...
  const fileData = await readFromFilePath(ORIGIN, true)
  if (fileData.length) {
    for await (const { users, from, to } of genUsers(fileData)) {
      ...
      try {
        await writeToFilePath(toUsersImportsFilePath(USERS_IMPORTS_PATH, fileName), users, true)

        const status = await createJob(users)

        await writeToFilePath(toStatusFilePath(DESTINATION, fileName), status, true)
